...
(async () => {
  ...
  const wb2 = await page.evaluate(async () => {
    // 注入 mock single-file handle (带 createWritable 模拟)
    const mockHandle = {
      name: 'mock.md',
      async queryPermission() { return 'granted'; },
      async requestPermission() { return 'granted'; },
      async getFile() { return { lastModified: Date.now() }; },
  ...
  const writeResult = await page.evaluate(async () => {
    ...
    // 单 .md 模式只写主文件 (sidecar 靠 IDB 缓存; 这版 tryWriteBack 不再写 sidecar JSON 文件)
  ...
  // Replaced by single-file flow coverage in TEST 1 + TEST 17 + TEST 37
  ...
  // 准备: 加载 sample.md 作 currentFile, 用作测试基础
  await page.evaluate((md) => {
  ...
  // 验证 badge text: handle mode → "已授权"
  const badgeText = await page.locator('#file-tree .save-mode-badge').textContent();
  console.log(`  ✓ badge 内容: "${badgeText?.trim()}"`);
  if (!badgeText?.includes('已授权')) throw new Error('handle 模式 badge 应含"已授权"');
