...
## v1.44.4 (2026-07-25) — 批注卡片 polish

用户: 「批注卡片的设计上还有咩有机会优化一下」

### 改动
1. **模式左边线** `--thread-rail`：人类 accent / AI / 审阅 / 已解决 一目了然（创建时锁定后的视觉身份）
2. **人类 badge 去噪**：默认人类不贴标；仅 AI / 审阅显示 badge
3. **quote 暗色安全**：去掉 `#faf9f6` 硬编码渐变，全走 token
4. **解决钮安静化**：ghost 文字「解决/重开」，去掉 ✓↺；删死 header 解决钮
5. **密度**：quote/form/reply 略收紧；textarea focus 不用 `#fff`

### 测试
- v143-ai-float-btn / e2e-annotation-color-system / 06-float
